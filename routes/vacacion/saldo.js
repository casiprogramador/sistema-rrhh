...
router.get('/formulario', function(req, res, next) {
    ...
    modelos.Area.findAll({
        ...
        modelos.sequelize.query('SELECT e.ndi, e.materno, e.paterno, e.nombres, a.cargo, c.fecha_inicio, s.gestion, s.dias , r.desc_area FROM public."Empleados" e, public."Contratos" c, public."Cargos" a , public."Areas" r, public."Saldo_Vacacions" s WHERE e.id=c.id_empleado and c.id_cargo=a.id and a.id_area=r.id and s.id_empleado = e.id').spread((results, metadata) => {
            console.log(JSON.stringify(results));
            res.render('vacacion/saldo', { areas: areas, saldovacaciones: results });
        })
